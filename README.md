...
- **Plan mode по Shift+Tab** — режимы `ask → accept-edits → plan`: в plan-режиме агент исследует read-only и приносит план; мутации технически запрещены, режим виден в промпте (`plan ❯`) и хранится в сессии.
- **`task`** — read-only сабагент: исследует кодовую базу в собственном контекстном окне и возвращает выводы, не засоряя основной диалог (как Agent/Explore).
- **Точные описания инструментов** — «когда использовать / когда нет» (именно это Anthropic называют главным рычагом качества агента).

## Интерфейс

- **Маскот Avi** 🐦 — собственная птичка Avery: в баннере рядом с метаданными сессии и в анимации «думает…» (моргает и косится)
- Анимированный баннер с градиентом при запуске
- **Markdown-рендер ответов**: заголовки, списки, цитаты, **таблицы** (выровненные колонки), блоки кода с **подсветкой синтаксиса** (ts/js/py/bash/json)
- **Боксы инструментов**: вызов и результат обрамляются линиями `╭─`/`╰─` с таймером
- Спиннер «думает…» с кадрами маскота
...
- **Tab** — автодополнение слэш-команд; **Shift+Tab** — режимы разрешений
...
- Футер после каждого ответа: модель, токены, стоимость, время
...
- **Режимы по Shift+Tab**: `ask` (спрашивать) → `accept-edits` (правки файлов без вопросов, команды спрашиваем) → `plan` (только исследование и план, мутации запрещены технически).
